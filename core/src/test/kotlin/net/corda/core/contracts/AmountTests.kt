...
class AmountTests {
    ...
    fun amountTransferApply() {
        ...
        val sourceAccounts = listOf(
                AmountTransfer.OwnerAndAmount(partyA, DOLLARS(123), 1),
                AmountTransfer.OwnerAndAmount(partyB, DOLLARS(100), 2),
                AmountTransfer.OwnerAndAmount(partyC, DOLLARS(123), 3),
                AmountTransfer.OwnerAndAmount(partyB, DOLLARS(100), 4),
                AmountTransfer.OwnerAndAmount(partyA, POUNDS(256), 5),
                AmountTransfer.OwnerAndAmount(partyB, POUNDS(256), 6)
        )
        val collector = Collectors.toMap<AmountTransfer.OwnerAndAmount<Currency, String>, Pair<String, Currency>, BigDecimal>({ Pair(it.owner, it.amount.token)}, {it.amount.toDecimal()}, { x,y -> x + y})
